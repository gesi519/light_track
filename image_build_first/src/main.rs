...
fn final_scene(image_width : usize, sample_per_pixel : usize, max_depth : usize) -> Result<(), std::io::Error> {
    ...
    for i in 0..20 {
        for j in 0..20 {
            ...
            boxes1.add(Quad::make_box(&Point3::new(x0, y0, z0), &Point3::new(x1, y1, z1), ground.clone()));
        }
    }
    ...
    world.add(Arc::new(Sphere::new_stationary(Point3::new(260.0, 150.0, 45.0), 50.0, Arc::new(Dielectric::new(1.5)))));
    world.add(Arc::new(Sphere::new_stationary(Point3::new(0.0, 150.0, 145.0), 50.0, Arc::new(Metal::new(Color::new(0.8, 0.8, 0.9), 1.0)))));

    let boundary = Arc::new(Sphere::new_stationary(Point3::new(360.0, 150.0, 145.0), 70.0, Arc::new(Dielectric::new(1.5))));
    world.add(boundary.clone());
    world.add(Arc::new(ConstantMedium::new_with_color(boundary, 0.2, Color::new(0.2, 0.4, 0.9))));
    let boundary = Arc::new(Sphere::new_stationary(Point3::new(0.0, 0.0, 0.0), 5000.0, Arc::new(Dielectric::new(1.5))));
    world.add(Arc::new(ConstantMedium::new_with_color(boundary, 0.0001, Color::new(1.0, 1.0, 1.0))));

    let emat = Arc::new(Lambertian::from_texture(Arc::new(ImageTexture::new("earthmap.jpg"))));
    world.add(Arc::new(Sphere::new_stationary(Point3::new(400.0, 200.0, 400.0), 100.0, emat)));
    let pertext = Arc::new(NoiseTexture::new(0.2));
    world.add(Arc::new(Sphere::new_stationary(Point3::new(220.0, 280.0, 300.0), 80.0, Arc::new(Lambertian::from_texture(pertext)))));

    let mut boxes2 = HittableList::new();
    let white = Arc::new(Lambertian::new(Color::new(0.73, 0.73, 0.73)));
    for _j in 0..1000 {
        boxes2.add(Arc::new(Sphere::new_stationary(Point3::random_range(0.0, 165.0), 10.0, white.clone())));
    }

    world.add(Arc::new(Translate::new(Arc::new(RotateY::new(Arc::new(BvhNode::new_from_list(&boxes2)), 15.0)),Vec3::new(-100.0, 270.0, 395.0))));
